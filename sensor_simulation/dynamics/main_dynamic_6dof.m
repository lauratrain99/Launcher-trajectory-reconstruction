%% 3 DOF simulation of launcher atmospheric phase
...
for i=1:length(t)-1
      q0_dot(i) = (q0(i+1)-q0(i))./(t(i+1)-t(i));
      q1_dot(i) = (q123Vect(i+1,1)-q123Vect(i,1))./(t(i+1)-t(i));
      q2_dot(i) = (q123Vect(i+1,2)-q123Vect(i,2))./(t(i+1)-t(i));
      q3_dot(i) = (q123Vect(i+1,3)-q123Vect(i,3))./(t(i+1)-t(i));
end

q_dot = [q0_dot; q1_dot; q2_dot; q3_dot];                                   % Quaternion Derivatives Vector
...
% grid minor
% xlabel('{t}  [s]')
% ylabel('{Q_{derivative}}')
% legend('q0_{der}', 'q1_{der}', 'q2_{der}', 'q3_{der}')
% title({'Quaternions Derivatives'})
...
% figure(9)
% plot(t(2:end), w_att(:,2:end))
% grid on
% grid minor
% xlabel('{t}  [s]')
% ylabel('{\omega}')
% legend('{\omega_{roll}}', '{\omega_{pitch}}', '{\omega_{yaw}}')
...
% legend('{acc_{x}}', '{acc_{y}}', '{acc_{z}}')
% title({'Non-gravitational accelerations'})
