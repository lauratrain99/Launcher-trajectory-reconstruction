...
global mu g0 Isp R_earth w_earth mdot P_e A_e A_a M0 cD0 m0 te_VR rHat ...
    pitch_deg uVect r0Vect acc_VR acc_CPR acc_CP acc_GT t_VRacc t_CPRacc t_CPacc t_GTacc
...
%     vdot_nongrav = Th/m*r/norm(r)- 0.5*rho*A_a/m*cD*v_r*norm(v_r);
    vdot_nongrav = -mu/norm(r)^3*r+Th/m*r/norm(r)- ...
           0.5*rho*A_a/m*cD*v_r*norm(v_r);
    t_VRacc = [t_VRacc; t];
    acc_VR = [acc_VR; vdot_nongrav'];
...
elseif phase_code == 2
    ...
    vdot = -mu/norm(r)^3*r+Th/m*uVect/norm(uVect)- ...
           0.5*rho*A_a/m*cD*v_r*norm(v_r);
%     vdot_nongrav = Th/m*uVect/norm(uVect)-  0.5*rho*A_a/m*cD*v_r*norm(v_r);
    vdot_nongrav = -mu/norm(r)^3*r+Th/m*uVect/norm(uVect)- ...
           0.5*rho*A_a/m*cD*v_r*norm(v_r);
    t_CPRacc = [t_CPRacc; t];
    acc_CPR = [acc_CPR; vdot_nongrav'];
...
%     vdot_nongrav = Th/m*uVect/norm(uVect)- 0.5*rho*A_a/m*cD*v_r*norm(v_r);
    vdot_nongrav = -mu/norm(r)^3*r+Th/m*uVect/norm(uVect)- ...
        0.5*rho*A_a/m*cD*v_r*norm(v_r);
    t_CPacc = [t_CPacc; t];
    acc_CP = [acc_CP; vdot_nongrav'];
    
elseif phase_code == 4    
    
    vdot = -mu/norm(r)^3*r+Th/m*v_r/norm(v_r)- ...
        0.5*rho*(A_a/m)*cD*v_r*norm(v_r);
%     vdot_nongrav = Th/m*v_r/norm(v_r)- 0.5*rho*(A_a/m)*cD*v_r*norm(v_r);
    vdot_nongrav = -mu/norm(r)^3*r+Th/m*v_r/norm(v_r)- ...
            0.5*rho*(A_a/m)*cD*v_r*norm(v_r);
    t_GTacc = [t_GTacc; t];
    acc_GT = [acc_GT; vdot_nongrav'];
